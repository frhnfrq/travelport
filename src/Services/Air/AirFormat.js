...
function formatSegment(segment) {
  ...
  if (segment["air:FlightDetails"]) {
    Object.assign(seg, {
      details: Object.keys(segment["air:FlightDetails"]).map((flightKey) => {
...
function formatLowFaresSearch(searchRequest, searchResult) {
  ...
  Object.entries(results).forEach(([fareKey, price]) => {
    ...
    const taxes = thisFare["air:TaxInfo"];

    Object.keys(taxes).forEach((taxKey) => {
      ...
      directions = thisFare["air:FlightOptionsList"].map((direction) =>
        Object.values(direction["air:Option"]).map((option) => {
          const trips = option["air:BookingInfo"].map((segmentInfo) => {
            ...
            const rawSegment = { ...segments[segmentInfo.SegmentRef] };

            // for (let key in rawSegment) {
            ...
            // console.log(
            //   JSON.stringify(
            //     transformers.processRawSegment({ segments: [{ rawSegment }] })
            //   )
            // );
...
const getSegmentsData = (segmentsObject) =>
  segmentsObject ? Object.values(segmentsObject) : null;
...
function setIndexesForSegments(
  ...
  const segments = segmentsData ? setIndexes(segmentsData) : null;
...
function setReferencesForSegments(segments) {
  return segments.map((segment, idx) => {
    ...
    const nextSegmentReference =
      currentGroup === nextGroup ? nextSegment.uapiSegmentReference : null;
