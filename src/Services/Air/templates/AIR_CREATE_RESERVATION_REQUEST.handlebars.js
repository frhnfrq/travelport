...
<soap:Envelope xmlns:soap="http://schemas.xmlsoap.org/soap/envelope/">
    ...
    <soap:Body>
        <univ:AirCreateReservationReq
            ...
            {{/if}}
            {{#each passengers}}
            <com:BookingTraveler Key="P_{{@index}}" BookingTravelerRef="{{ref}}" Age="{{Age}}" DOB="{{DOB}}" Gender="{{gender}}" TravelerType="{{travelerType}}">
                <com:BookingTravelerName Prefix="{{{prefix}}}" First="{{firstName}}" Last="{{lastName}}" {{#if title}}Prefix="{{title}}"{{/if}}/>
                {{#if (and ../deliveryInformation @first)}}
                <com:DeliveryInfo>
                    <com:ShippingAddress>
                ...
                {{#if isChild}}
                <com:NameRemark>
                    <com:RemarkData>P-{{remarkCode}} DOB{{dobString}}</com:RemarkData>
                </com:NameRemark>
                {{/if}}
                {{#if isInfant}}
                    <com:NameRemark>
